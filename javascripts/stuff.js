...
$.getJSON('./sources/github-community-repos.json', function (projects) {  
  ...
  Object.keys(projects).forEach( function (key) {
    ...
    var charHTML
    charHTML = "<div><a target='_blank' onclick=\"ga('send', 'event', 'click', 'Github: " + project['name'] + "'); return false;\" title='#" + counter + " " + project['name'] + "' href='" + 
...
$(document).on('source-github:ready', function () {  
  $.getJSON('./sources/share-zabbix.json', function (projects) {
    ...
    Object.keys(projects).forEach( function (key) {
      var project = projects[key]
      var charHTML
      charHTML = "<div><a target='_blank' onclick=\"ga('send', 'event', 'click', 'Share: " + project['name'] + "'); return false;\" title='#" + counter + " " + project['name'] + "' href='" + 
        project['url'] + "'><i class='fa fa-" + project['icon'] + "'></i> " + project['name'] + "</a></div>"
      container.append("<li class='result emoji-wrapper'>" +
        charHTML + "<span class='keywords'>" + project["keywords"] + "</span></li>")
      counter++
...
$(document).on('source-share:ready', function () {
  $.getJSON('./sources/zabbix-wiki.json', function (projects) {
    ...
    Object.keys(projects).forEach( function (key) {
      var project = projects[key]
      var charHTML
      charHTML = "<div><a target='_blank' onclick=\"ga('send', 'event', 'click', 'Wiki: " + project['name'] + "'); return false;\" title='#" + counter + " " + project['name'] + "' href='" + 
        project['url'] + "'><i class='fa fa-" + project['icon'] + "'></i> " + project['name'] + "</a></div>"
      container.append("<li class='result emoji-wrapper'>" +
        charHTML + "<span class='keywords'>" + project["keywords"] + "</span></li>")
      counter++
