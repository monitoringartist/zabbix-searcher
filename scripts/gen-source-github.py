...
def iconizing(name, keywords):
    ...
    if 'monitoringartist' in name:
        return 'star'
    if any(word in 'aws amazon ec ec2 elb rds' for word in keywords):
        return 'amazon'
    if any(word in 'bash sshd' for word in keywords):
        return 'terminal'
    if any(word in 'db database sql rabbitmq redis riak elastic elasticsearch informix postgresql memcached mysql zbxora' for word in keywords):
        return 'database'
    if any(word in 'grafana grafana-zabbix' for word in keywords):
        return 'bar-chart'
    if any(word in 'linux debian fedore redhat yum ubuntu' for word in keywords):
        return 'linux'
    if any(word in 'printer xerox kyocera workcenter laserjet' for word in keywords):
        return 'print'
    if any(word in 'wifi mikrotik ubiquiti airfiber' for word in keywords):
        return 'wifi'
    if any(word in 'postfix mail email exim zimbra' for word in keywords):
        return 'envelope'
    if any(word in 'java jvm tomcat jboss' for word in keywords):
        return 'cofee'
    if any(word in 'backup backupc array freenas raid qnap hdd disk netapp storwize purestorage synology hddtemp iostat emc storage veeam' for word in keywords):
        return 'hdd-o'
    if any(word in 'weather' for word in keywords):
        return 'bold'
    if any(word in 'hadoop hbase storm' for word in keywords):
        return 'table'
    if any(word in 'cisco juniper switch router asa' for word in keywords):
        return 'sitemap'
    if any(word in 'ntp' for word in keywords):
        return 'clock-o'
    if any(word in 'ups apc' for word in keywords):
        return 'battery-half'
    if any(word in 'vm kvm openvz xen vmware esxi server' for word in keywords):
        return 'server'
